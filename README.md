...
- [**`service.bat`**](./service.bat) - установка в автозапуск и другие функции:
  ...
  **После переключения требуется перезапуск стратегии.**  
  ...
  - **`Update Hosts File`** - обновление системного файла `hosts` (как правило, это файл `C:\Windows\System32\drivers\etc\hosts`) <ins>**для починки веб версии телеграма и подключения к голосовому чату Discord**</ins>
